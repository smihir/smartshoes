...
class Requester(GATTRequester):
    ...
    def on_notification(self, handle, data):
        ...
        if ay >= self.local_maxima:
            self.local_maxima = ay
        else:
            found_maxima = True

        if (found_maxima and self.local_maxima > ay_avg + 1):
            print("Jump...")
            m.click(x_dim/2, y_dim/2, 1)
        elif (found_maxima and self.local_maxima > ay_avg + 1.5):
            print("Double Jump...")
            m.click(x_dim/2, y_dim/2, 2)
